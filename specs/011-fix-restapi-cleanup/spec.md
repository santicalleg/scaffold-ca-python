...
**Feature Branch**: `011-fix-restapi-cleanup`  
**Created**: 2026-04-10  
**Status**: Draft  
...
- **FR-004**: When `gep --type restapi --dry-run` is invoked, the tool MUST report the scripts update in its preview output without modifying `pyproject.toml`. The dry-run preview MUST also indicate that `main.py` would be deleted.
...
- **SC-001**: After `gep --type restapi` completes, no test run may observe `src/<package>/main.py` present on disk.
